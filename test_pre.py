...
# from models.UNetDA import UNet_mpt,UNet
...
# from models.UNetAS_instance_noams import UNet,UNet_mpt
from models.UNetAS import UNet,UNet_mpt
...
def main(args):
    ...
    if args.type == 'v1':
        valid_dataset = SeismicDataset_kuang(args.input,start=10000, end=10090, phase='test')
    else:
        valid_dataset = SeismicDataset_kuang(args.input, start=10000, end=10090, phase='test')
    ...
    # model = torch.load('D:/X/p_wave/output/model_unet_mpt.pt')
    # model = torch.load('D:/X/p_wave/output/trans/全微调_unet/unet.pth')
    # model = torch.load('D:/X/p_wave/output/best_model.pt')

    # ##################################################

    model = UNet_mpt()  # 替换成你的模型结构定义
    state_dict = torch.load('D:/X/p_wave/output/best_model.pt', map_location='cpu')  # 如果是在 CPU 上加载
    ...
    # print("模型的键：", model.state_dict().keys())
    # print("检查点的键：", state_dict.keys())
    ...
    p_precision, p_recall = metric['P']['tp'] / (metric['P']['tp'] + metric['P']['fp'] + 1e-9), metric['P']['tp'] / (metric['P']['tp'] + metric['P']['fn'] + 1)
